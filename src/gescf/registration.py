...
def coarse_transform(dataset, img_size, img_t0, img_t1, gray_img_t0, gray_img_t1):
    ...
    default_sp = Path(__file__).resolve().parents[3] / "models" / "matching" / "superpoint_v1.pth"
    sp_ckpt = os.environ.get("EDGE_CD_SUPERPOINT_CKPT", str(default_sp))

    # load Superpoint extractor
    fe = SuperPointFrontend(weights_path=sp_ckpt,
                            nms_dist=3,
                            conf_thresh=0.015,
                            nn_thresh=0.7,
                            cuda=False)
